...
import pytest
import pandas as pd
from src.db import init_db, get_connection, query_to_df, DEFAULT_DB_PATH
...
def test_query_to_df_bindings(temp_db):
    ...
    df_param = query_to_df("SELECT * FROM sellers WHERE seller_id = ?", params=("SEL-001",), db_path=temp_db)
